...
function FeedbackItem({ item, pending, onStatusChange, onNoteSave }: { item: SharedFeedbackItem; pending: boolean; onStatusChange: (status: DevLogStatus) => void; onNoteSave: (note: string) => Promise<void> }) {
  ...
  const saveNote = async () => {
    if (noteSaving || noteDraft === item.note) return;
    setNoteSaving(true); setNoteError(null);
    try { await onNoteSave(noteDraft); } catch { setNoteError("Poznámku se nepodařilo uložit."); }
  ...
  return <article className={`dev-log-item status-${item.status}${open ? " is-open" : ""}`}>
    <div className="dev-log-item-row">
      <button className="dev-log-item-toggle" type="button" aria-expanded={open} aria-controls={contentId} onClick={() => setOpen((value) => !value)}>
        <span className="dev-log-item-meta"><time dateTime={item.createdAt}>{dateFormatter.format(new Date(item.createdAt))}</time>{item.note.trim() && <MessageCircle className="dev-log-message-indicator" role="img" aria-label="Záznam obsahuje poznámku" />}</span><strong>{item.title}</strong><ChevronDown className="dev-log-chevron" aria-hidden="true" />
      </button>
    ...
    {open && <div className="dev-log-item-content" id={contentId}>
      {item.note.trim() && <section className="dev-log-note"><h4>Developer message</h4><p>{item.note}</p></section>}
      {item.details.map((detail, index) => <section key={`${detail.label}-${index}`}><h4>{detail.label}</h4><p>{detail.text}</p></section>)}
      <section className="dev-log-note-editor"><label htmlFor={`dev-log-note-${item.id}`}>Poznámka</label><textarea id={`dev-log-note-${item.id}`} maxLength={2000} value={noteDraft} disabled={noteSaving} onChange={(event) => setNoteDraft(event.target.value)} /><div className="dev-log-note-actions">{noteError && <span role="alert">{noteError}</span>}<button type="button" disabled={noteSaving || noteDraft === item.note} onClick={() => void saveNote()}>{noteSaving ? "Ukládám…" : "Uložit"}</button></div></section>
    </div>}
  </article>;
}
...
export function DevLogPanel({ result, open, onClose }: { result: SharedFeedbackResult; open: boolean; onClose: () => void }) {
  ...
  const saveNote = async (id: string, note: string) => {
    const response = await fetch("/api/devlog-state", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, note }) });
    const body = await response.json() as unknown;
    const stored = response.ok ? parseDevLogStateOverride(body) : null;
    if (!stored) throw new Error("persistence_failed");
    setItems((current) => current.map((item) => item.id === id ? mergeDevLogState(item, stored) : item));
