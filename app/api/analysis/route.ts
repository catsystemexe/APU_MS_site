...
export async function POST(request: Request) {
  ...
  try {
    ...
    const callId = usage_record.call_id;
    ...
    const developerData = identity.role === "developer" ? { diagnostics: { callId, canonicalCallIds: [usage_record.call_id], model, reasoning: "low", knowledgeBaseEnabled: true, routingSource: "phase-2", inputTokens, cachedInputTokens: usage?.input_tokens_details?.cached_tokens ?? 0, outputTokens, reasoningTokens: usage?.output_tokens_details?.reasoning_tokens ?? 0, totalTokens: usage?.total_tokens ?? inputTokens + outputTokens, estimatedCostUsd: estimateCostUsd({ model, inputTokens, cachedInputTokens: usage?.input_tokens_details?.cached_tokens ?? 0, outputTokens, fileSearchCalls }) }, telemetry: {
