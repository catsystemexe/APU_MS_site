...
function validSnapshot(value: unknown): value is F2PreviewSnapshot { if (!value || typeof value !== "object") return false; const snapshot = value as Partial<F2PreviewSnapshot>; return isPath(snapshot.activePath) && snapshot.canonicalNeed?.needText !== undefined && snapshot.processedRevision === snapshot.buildRevision && snapshot.processedBuild?.path === snapshot.activePath && Array.isArray(snapshot.hypotheses) && Array.isArray(snapshot.activeSkills); }
