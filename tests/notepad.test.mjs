...
test("notepad uses local persistence and floating Lucide tool buttons", () => {
  ...
  assert.match(page, /activeWorkspacePanel: activePanel/);
  assert.match(page, /composer-workspace-indicator/);
  ...
  assert.match(page, /notepad: "Doplňte Zápisník…"/);
  assert.match(page, /analysis: "Upřesněte Rozbor…"/);
  assert.match(page, /output: "Upravte Výstup…"/);
  assert.match(page, /placeholder=\{inputPlaceholder\}/);
  assert.match(page, /\{!isComposerExpanded && <span>\{inputPlaceholder\}<\/span>\}/);
  assert.match(page, /Aktivní kontext:/);
